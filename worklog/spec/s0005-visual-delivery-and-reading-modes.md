...
- Provide a browsable contents list and a separately addressable article for each of the 47 catalogue entries in n0022, preserving their order and using Korean subject titles. This replaces the former single dummy topic page.
...
- Article descriptions and bodies MUST remain empty until content is separately requested. Remove the sample document, sample prose, code, and demonstration content. Empty pages MUST omit section navigation and filler while retaining their title and ordinary page navigation.
...
- The context-window demonstration MUST offer user-controlled animation showing input, internal reasoning, and visible answer tokens filling the available space. Its output-budget scale MUST remain invariant across scenarios and explicitly show unused capacity, including capacity unavailable because of the context limit. Scenario changes, playback, and response comparisons MUST avoid sudden layout shifts; preserve readable static print output and reduced-motion usability.
